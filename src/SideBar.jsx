...
export default function SideBar({ activeItem, onNavigate }) {
  ...
  const renderNavButton = (item) => {
    ...
    return (
      ...
      >
        ...
        />
        {!collapsed && (
          <span className="hidden text-[14px] font-medium truncate md:inline">
            {item.label}
          </span>
        )}
      </button>
  ...
  return (
    ...
    >
      {/* Brand */}
      <div
        className={`flex items-center border-b border-slate-800/60 justify-center px-0 py-4 ${
          collapsed ? "md:justify-center md:px-0 md:py-4" : "md:gap-2.5 md:px-4 md:py-4"
        }`}
      >
      ...
      {/* Collapse / expand toggle — only meaningful once there's room to collapse/expand, so it's desktop-only */}
      <div className="hidden px-2.5 pb-3 md:block">
        <button
          type="button"
